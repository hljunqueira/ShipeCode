...
import { Project, ProjectStatus, TaskStatus, Task } from '../types';
import { CONTRACT_TEMPLATE, PROJECT_PHASES, PROJECT_STATUS_LABELS } from '../constants';
import { CheckCircle, Clock, FileText, DollarSign, List, ShieldCheck, Layout, GripVertical, Plus, User as UserIcon, Activity, Edit3 } from 'lucide-react';
...
const ProjectDetail: React.FC<ProjectDetailProps> = ({ project, onUpdateProject }) => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'board' | 'finance' | 'contract'>('board');
  const scrollRef = useDraggableScroll();
  const [taskModalOpen, setTaskModalOpen] = useState(false);
  const [editingTask, setEditingTask] = useState<Task | undefined>(undefined);
  const [newTaskColumn, setNewTaskColumn] = useState<TaskStatus | undefined>(undefined);
  ...
  const handleGenerateContract = () => {
    const template = CONTRACT_TEMPLATE
      .replace(/{ORG_NAME}/g, organization.name)
  ...
  const handleSaveTask = (task: Task) => {
    ...
    if (existingTaskIndex >= 0) {
      ...
      // Nova tarefa com status da coluna selecionada
      const newTask = { ...task, status: newTaskColumn || TaskStatus.TODO };
  ...
  const handleTaskClick = (task: Task, e: React.MouseEvent) => {
    // Só abre o modal se não for um drag
    if (e.detail === 1) {
      setTimeout(() => {
        setEditingTask(task);
        ...
        <div className="flex gap-6 overflow-x-auto hide-scrollbar w-full sm:w-auto">
          {(Object.keys(tabLabels) as Array<keyof typeof tabLabels>).map((tab) => (
            <button
              ...
              className={`pb-3 text-sm font-medium transition-all relative whitespace-nowrap ${activeTab === tab
                ? 'text-white'
                    ...
                    <div className={`flex flex-col gap-3 min-h-[400px] rounded-2xl p-2 border-2 border-dashed border-transparent transition-all ${tasks.length === 0 ? 'bg-zinc-900/20' : ''}`}>
                      ...
                      {tasks.map(task => (
                        <div
                          ...
                          className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl shadow-lg hover:border-zinc-500/50 hover:shadow-2xl hover:-translate-y-1 transition-all cursor-pointer group/card relative overflow-hidden"
                        >
                          {/* Decoration */}
                          <div className={`absolute top-0 left-0 w-1 h-full transition-colors ${task.priority === 'high' ? 'bg-red-500' : task.priority === 'medium' ? 'bg-amber-500' : 'bg-zinc-800 group-hover/card:bg-red-500'}`}></div>

                          <div className="pl-2">
                            <div className="flex justify-between items-start mb-2">
                              <span className="text-[10px] font-mono text-zinc-500">{task.id}</span>
                              <Edit3 size={12} className="text-zinc-700 group-hover/card:text-zinc-400" />
                            </div>
                      ...
                      <button
                        ...
                        className="w-full py-3 mt-2 border border-dashed border-zinc-800 rounded-xl text-zinc-600 text-xs font-bold uppercase tracking-wider hover:text-white hover:border-zinc-600 hover:bg-zinc-900 transition-all flex items-center justify-center gap-2 opacity-60 hover:opacity-100"
                      >
